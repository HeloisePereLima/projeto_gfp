...
export default function CategoriasModal({ modalAberto, fecharModal, itemAlterar }) {
    ...
    const botaoSalvar = async () => {
        ...
        try {
            ...
            if (itemAlterar) {
                endpoint = `${enderecoServidor}/categorias/${itemAlterar.id_conta}`
                metodo = 'PUT'
            }
            ...
            if (resposta.ok) {
                alert('Categorias cadastrada com sucesso!')
                fecharModal()
            }
        ...
        <div className='fixed inset-0 bg-black/80 py-6 px-4 flex justify-center items-center z-50'>
            <section className='w-full max-w-2xl bg-white p-8 rounded-lg shadow-lg text-gray-800'>
                ...
                <div className='space-y-5'>
                    <div className='space-y-5'>
                        <div className='flex rounded-md shadow-sm'>
                            <button type='button' onClick={() => setTipoTransacao('ENTRADA')}
                                className={`flex-1 p-2 rounded-l-md ${tipoTransacao == 'ENTRADA' ? 'bg-green-500 text-white' : 'bg-gray-200'}`}>ENTRADA</button>
                            <button type='button' onClick={() => setTipoTransacao('SAIDA')}
                                className={`flex-1 p-2 rounded-r-md ${tipoTransacao == 'SAIDA' ? 'bg-red-500 text-white' : 'bg-gray-200'}`}>SAIDA</button>
                        </div>
                    </div>
                    ...
                    <label className={Estilos.labelCadastro}>Escolha um cor </label>
                    ...
                    <div className='flex justify-end gap-3 mt-8'>
                        ...
                        <button className={Estilos.botaoOutline} onClick={botaoSalvar}>
                            <MdClose />Salvar
                        </button>
                    </div>
                </div>
